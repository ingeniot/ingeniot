...
if(isset($_POST['email']) && isset($_POST['password'])) {
  ...
  if ($_POST['email']==""){
    ...
    $conn = mysqli_connect($nombre_servidor,$nombre_usuario,$clave,$nombre_base_datos);
...
<!-- core -->
  ...
  //const WebSocket_URL = 'ws://ingeniot.tk:8093/mqtt' //sin ssl
  const WebSocket_URL = 'wss://ingeniot.tk:8094/mqtt' //con ssl
  // TCP/TLS connect url
  ...
  client.on('connect', () => {
      ...
      console.log('Conexión exitosa de MQTT')
      ...
      if (!error) {
          console.log('Subscripción exiotosa')
          client.publish('edificio', 'Aplicación suscripta', { qos: 1, rein: false }, (error) => {
                  console.log(error || 'Publicación exitosa')
              })
      }
              else {
    ...
    //  }
      ...
      client.publish('edificio', 'Temperatura edificio 1', (error) => {
          console.log(error || 'Publicación exitosa')
      })
  ...
  client.on('reconnect', (error) => {
      ...
      console.log('Eror al reconectar', error)
  })
