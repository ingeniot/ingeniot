...
include_once "./ValidadorTableroEditado.inc.php";
...
if (isset($_POST["nombre"])) {  //Hace la validación si se pulso el boton Enviar datos
  ...
  $validador = new ValidadorTableroEditado($_POST["nombre"], $_POST["nombre_original"], $_POST["tipo"], $_POST["tipo_original"],Conexion::obtener_conexion()); //Crea una instancia para validacion de los datos enviados
  if ($validador->registro_valido()) { //Verifica que todos los datos ingresados sean validos
    $tablero_editado = RepositorioTablero::actualizar_tablero(Conexion::obtener_conexion(), $id, $validador->obtener_nombre(),$validador->obtener_tipo());
    if(!$tablero_editado){
