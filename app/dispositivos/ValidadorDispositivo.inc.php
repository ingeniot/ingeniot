...
abstract class ValidadorDispositivo {   //Cons abstract esta clase no se puede instanciar
    ...
    protected function validar_nombre($conexion, $nombre) {
        ...
        if (!$this->variable_iniciada($nombre)) {
            return "Debes escribir un nombre de dispositivo";
        } else {
            $this->nombre = $nombre;
        }
        if (strlen($nombre) < 1) {
        ...
        if (RepositorioDispositivo::nombre_existe($conexion, $nombre)) {
            return "El nombre de dispositivo ya esta en uso";
        }
        return"";
    }
    protected function validar_tipo($conexion, $tipo) {
        ...
        if (!$this->variable_iniciada($tipo)) {
            return "Debes escribir un tipo de dispositivo";
        } else {
            $this->tipo = $tipo;
        }
        if (strlen($tipo) < 1) {
