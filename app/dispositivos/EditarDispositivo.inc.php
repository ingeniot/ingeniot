...
include_once "./ValidadorDispositivoEditado.inc.php";
...
if (isset($_POST["nombre"])) {  //Hace la validación si se pulso el boton Enviar datos
  ...
  $validador = new ValidadorDispositivoEditado($_POST["nombre"], $_POST["nombre_original"], $_POST["tipo"], $_POST["tipo_original"],
  ...
  if ($validador->registro_valido()) { //Verifica que todos los datos ingresados sean validos
    $dispositivo_editado = RepositorioDispositivo::actualizar_dispositivo(Conexion::obtener_conexion(),$id=$_POST["id"], $validador->obtener_nombre(),
    $validador->obtener_tipo(),$validador->obtener_etiqueta(),$validador->obtener_descripcion());
    if(!$dispositivo_editado){
