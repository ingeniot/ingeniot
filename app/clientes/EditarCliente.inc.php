...
include_once "./ValidadorClienteEditado.inc.php";
...
if (isset($_POST["nombre"])) {  //Hace la validación si se pulso el boton Enviar datos
  ...
  $validador = new ValidadorClienteEditado($_POST["nombre"], $_POST["nombre_original"], $_POST["tipo"], $_POST["tipo_original"],Conexion::obtener_conexion()); //Crea una instancia para validacion de los datos enviados
  if ($validador->registro_valido()) { //Verifica que todos los datos ingresados sean validos
    $cliente_editado = RepositorioCliente::actualizar_cliente(Conexion::obtener_conexion(), $id, $validador->obtener_nombre(),$validador->obtener_tipo());
    if(!$cliente_editado){
